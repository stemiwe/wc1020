...
<table id="table">
  ...
  <tbody>
    ...
    foreach ($rows as $row) {
        ...
        // Get MVP.
        $mvp_stats = get_medals($date)[0];
        $mvp = $mvp_stats['player'];
        $mvp_elo = $mvp_stats['elo'];
        ...
        echo '<td class="player-cell">' . write_player($mvp) . '</td>';
        echo '<td class="elo-cell elo-gain">+' . $mvp_elo . '</td>';
        echo '</tr>';
    }
    ?>
  </tbody>
